...
void
change(char *funcName, int queueType, int procCount, int runCount, int addDel)
{
	//Process queue
   if(queueType){
		//Process is added
		if(addDel){
	   	print("process added to process queue %s\n", funcName);
   	   print("process Queue Count - %d\n", procCount);
   	   print("run Queue Count - %d\n", runCount);
		}
		//Process is deleted 
		else{
	   	print("process deleted from process queue %s\n", funcName);
   		print("process Queue Count - %d\n", procCount);
   	   print("run Queue Count - %d\n", runCount);
		}
	}
	//Run queue
		...
		//Process is added
 		if(addDel){
	   	print("process added to run queue %s\n", funcName);
   	   print("process Queue Count - %d\n", procCount);
   	   print("run Queue Count - %d\n", runCount);
		}
		//Process is deleted
		else{
	   	print("process deleted from run queue %s\n", funcName);
   	   print("process Queue Count - %d\n", procCount);
   	   print("run Queue Count - %d\n", runCount);
			//Condition to check if scheduler is empty (run queue process count is 0)
	    	if(runCount == 0){
      		print("Run Queue Count is 0\n");
      		print("rQ0 - process Queue Count: %d\n", procCount);
      		print("rQ0 - run Queue Count: %d\n", runCount);
   	   }
		}  
	}
}
...
newprog(Prog *p, Modlink *m)
{
	...
	isched.tail = n;
   isched.processQueueCount++;
	change("newprog", 1, isched.processQueueCount, isched.runQueueCount, 1);
	ph = pidlook(n->pid);
	if(*ph != nil)
		panic("dup pid");
	n->pidlink = nil;
...
delprog(Prog *p, char *msg)
{
	...
	isched.processQueueCount--;
	change("delprog", 1, isched.processQueueCount, isched.runQueueCount, 0);
	ph = pidlook(p->pid);
	if(*ph == nil)
		panic("lost pid");
	*ph = p->pidlink;
	...
	isched.runQueueCount--;
	change("delprog", 0, isched.processQueueCount, isched.runQueueCount, 0);
	p->state = 0xdeadbeef;
	free(o->user);
	free(p->killstr);
	free(p->exstr);
...
addrun(Prog *p)
{
	...
	isched.runtl = p;
	isched.runQueueCount++;
	change("addrun", 0, isched.processQueueCount, isched.runQueueCount, 1);
}
...
delrun(int state)
{
	...
	if(p->link == nil)
		isched.runtl = nil;
	isched.runQueueCount--;
	change("delrun", 0, isched.processQueueCount, isched.runQueueCount, 0);
	return p;
}
...
delrunq(Prog *p)
{
	...
	if(p == isched.runtl)
		isched.runtl = prev;
	isched.runQueueCount--;
	change("delrunq", 0, isched.processQueueCount, isched.runQueueCount, 0);
}
...
acquire(void)
{
	...
	if(p->link == nil)
		isched.runtl = p;
	isched.runQueueCount++;
	change("acquire", 0, isched.processQueueCount, isched.runQueueCount, 1);
	strcpy(up->text, "dis");
}
...
disinit(void *a)
{
	...
	print("Date %ld\n", kerndate);
	hours = (kerndate/3600);
	mins = (kerndate -(3600*hours))/60;
	sec = (kerndate -(3600*hours)-(mins*60));
	print("H:M:S - %d:%d:%d\n",hours,mins,sec);
	if(vflag)
		print("Initial Dis: \"%s\"\n", initmod);
...
pushrun(Prog *p)
{
	...
	if(p->link == nil)
		isched.runtl = p;
	isched.runQueueCount++;
	change("pushrun", 0, isched.processQueueCount, isched.runQueueCount, 1);
